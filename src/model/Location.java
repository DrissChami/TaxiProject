...
public class Location {
	...
	/**
	 * Generate the next location to visit in order to reach the destination.
	 * 
	 * @param destination
	 *            Where we want to get to.
	 * @return The next location in a direct line from this to destination; or
	 *         destination if already arrived.
	 */
	public Location nextLocation(Location destination) {
		int moveX = destination.getX() - this.x;
		int moveY = destination.getY() - this.y;

		if (Math.abs(moveX) == Math.abs(moveY)) {
			return new Location(this.x + (moveX / Math.abs(moveX)), this.y
					+ (moveY / Math.abs(moveY)));
		} else if (Math.abs(moveY) > Math.abs(moveX)) {
			return new Location(this.x, this.y + (moveY / Math.abs(moveY)));
		} else {
			return new Location(this.x + (moveX / Math.abs(moveX)), this.y);
		}
	}
